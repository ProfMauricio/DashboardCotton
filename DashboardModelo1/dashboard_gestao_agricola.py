...
df_indices_solo = None
df_indices_calculados = None
df_fases_cultura = None
shape_file = None
...
def load_data_from_db():
    ...
    rdf_indices_solo = pd.read_sql_query("select * from dashboard.v_indices_solo", sessao)
    rdf_indices_calculados = pd.read_sql_query("select * from dashboard.v_indices_calculados", sessao)
    rdf_fases_cultura = pd.read_sql_query("select id, nome from dashboard.fases_cultura", sessao)
    sessao.close()
    engine.dispose()
    return rdf_indices_solo,rdf_indices_calculados,rdf_fases_cultura
...
def load_shapefile() :
    ...
    return gpd.read_file(shape_file_name)
...
# principal com cache e base de dados
if __name__ == '__main__':
    # iniciando a base de dados
    df_indices_solo, df_indices_calculados, df_fases_cultura = load_data_from_db()
    shape_file = load_shapefile()
    ...
    etapa_selecionada = st.sidebar.selectbox("Dados de qual etapa",
                                             df_fases_cultura['nome'].unique())


    df_dados_agro = df_indices_calculados[df_indices_calculados['fase_cultura'] == etapa_selecionada]
    #df_dados_agro.rename(columns={'bloco': 'FID'})

    dados_shape_mesclados = shape_file.merge(df_dados_agro, on='FID', how='inner')
    ...
    col1.markdown(f'<div style="text-align: center"><b>NVDI por bloco</b> ({etapa_selecionada})</div>',
                  unsafe_allow_html=True)
    ...
    dados_gestao_agro_termal = df_indices_calculados
    # transformando categoria para valor
    dados_gestao_agro_termal.loc[dados_gestao_agro_termal['status_termal'] == 'baixa', 'status_term_valor'] = 10
    dados_gestao_agro_termal.loc[dados_gestao_agro_termal['status_termal'] == 'media', 'status_term_valor'] = 20
    dados_gestao_agro_termal.loc[dados_gestao_agro_termal['status_termal'] == 'alta', 'status_term_valor'] = 30

    # st.write(dados_gestao_agro_termal)

    tipos_status_term = dados_gestao_agro_termal['status_termal'].value_counts()
    ...
    if tipos_status_term['status_termal'].value_counts().get('baixa', 0) > 0:
        custom_colors_termal.append('green')

    if tipos_status_term['status_termal'].value_counts().get('media', 0) > 0:
        custom_colors_termal.append('yellow')

    if tipos_status_term['status_termal'].value_counts().get('alta', 0) > 0:
    ...
    dados_gestao_agro = df_indices_calculados
    dados_gestao_agro['ndvi_cores'] = df_indices_calculados['ndvi'].map(
        lambda x: 'red' if x < valor_filtro_ndvi else 'green')
    # gráficos de barras de NDVI
    fig_ndvi = px.bar(dados_gestao_agro, y='ndvi', x="FID", height=alturaPadrao,
    ...
    col1.markdown(f'<div style="text-align: center"><b>Blocos abaixo do limiar</b> ({etapa_selecionada})</div>',
                  unsafe_allow_html=True)
    ...
    fig_status_termal = px.bar(
        ...
        labels={'status_termal': 'Tipos de status termal', 'count': 'Quantidade de blocos'},
    )
    ...
    col1.markdown(
        f'<div style="text-align: center"><b>Uniformidade de cultura por sítio específico</b> ({etapa_selecionada})</div>',
        unsafe_allow_html=True)
    ...
    col2.markdown(f'<div style="text-align: center"><b>Estimativa de safra por bloco</b> ({etapa_selecionada})</div>',
                  unsafe_allow_html=True)
    ...
    fig_barras_tx_ocupacao = px.bar(dados_gestao_agro,
                                    ...
                                    height=alturaPadrao,
                                    labels={'bloco': 'Bloco', 'taxa_ocupacao': 'Taxa de ocupação por bloco'},
                                    # title=f"Valores médios de SAVI por bloco no voo ({etapa_selecionada})",
                                    # range_y=[0,1],
                                    )
    ...
    elementos_baixo_A = df_indices_solo[df_indices_solo['horizonte'] == "A"]

    fig_status_elementosA_shp, ax_status_elementosA = plt.subplots(1, 1, figsize=(10, 10))

    item = col4.selectbox("Elementos horizonte A", ['cálcio', 'manganês', 'materia_organica', 'ferro'])

    dados_shape_mesclados_solo = shape_file.merge(df_indices_solo, on='FID', how='inner')
    dados_shape_mesclados_solo.plot(ax=ax_status_elementosA,
                               column=item,  # 'Mn_sB',
                               cmap='YlOrRd',
                               legend=True,
                               # legend_kwds={'label': 'Valores de status termal', 'orientation': 'vertical'},
    ...
    col4.markdown(
        f'<div style="text-align: center"><b>Indicativo de demanda por fertilização</b> ({etapa_selecionada})</div>',
        unsafe_allow_html=True)
